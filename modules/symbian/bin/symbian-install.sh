...
{
	cat << EOF
symbian-install script version $SCRIPT_VERSION
EOF
}
...
if [ ! -e $arg_source ]
then
	echo "Error: file or directory '$arg_source' not found"
	exit 1
fi

if [ -f $arg_source ]
then
	install_sis $arg_source
fi

if [ -d $arg_source ]
then
	echo -e "Argument is a directory - searching for SIS files ...\n"
	for sis in `find $arg_source -iname '*.sis*'`
	do
		install_sis $sis
	done
fi
