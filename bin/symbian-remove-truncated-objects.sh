...
# symbian-remove-truncated-objects

# If armcc is terminated while running, it can occasionally leave truncated
# object files in the /epoc32/build directory.  These binaries either have zero
# length, or only contain the ELF header (52 bytes).
#
# This script locates and removes these binaries.
...
{
	cat << EOF
symbian-remove-truncated-objects script
...
{
	cat << EOF
symbian-remove-truncated-objects script version $SCRIPT_VERSION
EOF
}
...
{
	size=$1
	cmd="find $epoc32_dir -iname *.o -size ${size}c"
	test "$option_verbosity" != silent && echo -e "\n$cmd"
	$cmd | while read file
	do
		test "$option_verbosity" != silent && echo $file
		test "$option_dryrun" == no && rm -f $file
	done
}
...
epoc32_dir=$EPOCROOT/epoc32/build

test "$option_verbosity" != silent && echo -e "\nepoc32 directory = $epoc32_dir"
